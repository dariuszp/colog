...
function Log() {
    ...
    this.info = function (message) {
        return this.log(this.apply(message, ['bold', 'colorWhite']));
    };

    this.success = function (message) {
        return this.log(this.apply(message, ['bold', 'colorGreen']));
    };

    this.warning = function (message) {
        return this.log(this.apply(message, ['bold', 'colorYellow']));
    };

    this.error = function (message) {
        return this.log(this.apply(message, ['bold', 'colorRed']));
    };

    this.question = function (message) {
        return this.log(this.apply(message, ['bold', 'colorCyan']));
    };

    this.answer = function (message) {
        return this.log(this.apply(message, ['bold', 'colorMagenta']));
    };

    this.headerInfo = function (message) {
        return this.log(this.apply(message, ['bold', 'colorBlack', 'backgroundWhite']));
    };

    this.headerSuccess = function (message) {
        return this.log(this.apply(message, ['bold', 'colorGreen', 'backgroundGreen']));
    };

    this.headerWarning = function (message) {
        return this.log(this.apply(message, ['bold', 'colorBlack', 'backgroundYellow']));
    };

    this.headerError = function (message) {
        return this.log(this.apply(message, ['bold', 'colorWhite', 'backgroundRed']));
    };

    this.headerQuestion = function (message) {
        return this.log(this.apply(message, ['bold', 'colorWhite', 'backgroundCyan']));
    };

    this.headerAnswer = function (message) {
        return this.log(this.apply(message, ['bold', 'colorWhite', 'backgroundMagenta']));
    };
    ...
    this.logo = function () {
        this.log(util.format('%s', this.backgroundGreen('|----------------------------|')));
        this.log(util.format('%s', this.backgroundGreen('|                            |')));
        this.log(util.format('%s %s %s', this.backgroundGreen('|     '), '                ', this.backgroundGreen('     |')));
        this.log(util.format('%s %s %s', this.backgroundGreen('|     '), this.veryRandomText('Półtorak Dariusz', 3), this.backgroundGreen('     |')));
        this.log(util.format('%s %s %s', this.backgroundGreen('|     '), '                ', this.backgroundGreen('     |')));
        this.log(util.format('%s', this.backgroundGreen('|                            |')));
        this.log(util.format('%s', this.backgroundGreen('|----------------------------|')));
    };
